...
// [[Rcpp::export]]
Rcpp::NumericVector coxph_cure_reg(
    const arma::vec& time,
    const arma::vec& event,
    const arma::mat& cox_x,
    const arma::mat& cure_x,
    const bool cure_intercept = true,
    const double& cox_lambda = 0,
    const arma::vec& cox_penalty_factor = 0,
    const double& cure_lambda = 0,
    ...
    )
{
    ...
    if (cox_start.n_elem == cox_x.n_cols) {
        ...
        tmp_object.regularized_fit(cox_lambda, cox_penalty_factor,
    ...
    if (cure_start.n_elem == cure_xx.n_cols) {
        ...
        cure_object.regularized_fit(cure_lambda, cure_penalty_factor,
    ...
    while (i < em_max_iter) {
        ...
        // M-step for the survival layer
        cox_object.set_offset(log_v);
        cox_object.regularized_fit(cox_lambda, cox_penalty_factor,
                                   cox_beta, cox_mstep_max_iter,
                                   cox_mstep_rel_tol);
